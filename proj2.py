...
def preprocess(img):
    ...
    kernel = np.ones((5, 5))
    img_dial = cv2.dilate(img_edge, kernel , iterations = 2)
...
def reorder(points):
    ...
    return new_points
    # width = points[np.argmax(add)][0] - points[np.argmin(add)][0]
    # height = points[np.argmax(add)][1] - points[np.argmin(add)][1]
    # return new_points, width, height


def warp(img, points):
    # points, width, height = reorder(points)
